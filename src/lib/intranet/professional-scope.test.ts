...
describe('professionalNameOwns', () => {
  ...
  it('casa Romeu dentro de lista Avec multi-profissional', () => {
    expect(professionalNameOwns('DORA,ROMEU FELIPE', 'Romeu Felipe')).toBe(true)
    expect(professionalNameOwns('ALINE BISPO PEREIRA,ROMEU FELIPE', 'Romeu Felipe')).toBe(true)
    expect(professionalNameOwns('GABRIELA SANTOS (ROMEU),ROMEU FELIPE', 'Romeu Felipe')).toBe(true)
  })
  ...
  it('não casa profissional errado', () => {
    ...
    expect(professionalNameOwns('DORA,ALINE BISPO PEREIRA', 'Romeu Felipe')).toBe(false)
  })
})

describe('filterByProfessionalName', () => {
  it('mantém só a agenda do profissional (inclui variante frouxa e lista)', () => {
    const rows = [
      ...
      { id: '5', professional_name: 'DORA,ALISON ALVAREZ' },
    ]
