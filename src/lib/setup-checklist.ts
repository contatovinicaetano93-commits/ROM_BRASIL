...
export const SETUP_ITEMS: SetupItem[] = [
  {
    ...
    steps: [
      ...
      'DATABASE_URL = Postgres dedicado só desta unidade (Brasil: Supabase; Iguatemi: Neon)',
  ...
  },
  {
    ...
    priority: 'quando_tiver',
    steps: [
      'Meta Developer → app → WhatsApp → API Setup',
      'Copiar Temporary/Permanent Token → WHATSAPP_CLOUD_TOKEN',
      'Copiar Phone number ID → WHATSAPP_PHONE_NUMBER_ID',
      'Definir WHATSAPP_VERIFY_TOKEN (string aleatória) e WHATSAPP_APP_SECRET (App Secret)',
      'Webhook Meta → https://seu-dominio/api/webhooks/whatsapp (subscribe: messages)',
      'Opcional aftercare fora de 24h: criar template aprovado → WHATSAPP_TEMPLATE_AFTERCARE',
      'Opcional: WHATSAPP_TEMPLATE_LANG=pt_BR',
