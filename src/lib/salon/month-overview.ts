...
export function analyticsFromMonthRow(row: SalonMonthMetricsRow): PeriodAnalytics {
  ...
  return {
    ...
    acquisition: [],
    return_rate: null,
    // Não inventar a partir de sum(day.new_clients) — mix diário não é 1ª visita.
...
export function applyWindowTotalsToOverview(
  ...
  const curKnown = knownDaily(current.totals)
  ...
  const monthRevenue = curKnown ? current.totals.revenue : null
  const monthAttended = curKnown ? current.totals.attended : null
  const prevRevenue = prevKnown ? previous.totals.revenue : null
  const prevAttended = prevKnown ? previous.totals.attended : null
  return {
    ...overview,
    label: currentLabel,
    ...
    analytics: {
      ...
      previous: {
        ...
        occupancy_avg: overview.analytics.previous?.occupancy_avg ?? null,
        packages_revenue: overview.analytics.previous?.packages_revenue ?? null,
        new_clients_period: overview.analytics.previous?.new_clients_period ?? null,
        return_rate: overview.analytics.previous?.return_rate ?? null,
      },
    },
    closing: {
      ...overview.closing,
      ...
      cash_flow:
        monthRevenue != null && monthRevenue > 0 ? currentBucket.cash_flow : null,
    },
    ...
    previous_closing: {
      ...
      cash_flow:
        prevRevenue != null && prevRevenue > 0 ? previousBucket.cash_flow : null,
      lost_revenue: prevLost,
    },
  }
}
