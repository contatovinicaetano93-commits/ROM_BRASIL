...
export async function GET(req: NextRequest) {
  try {
    ...
    return ok({
      ...
      cron: {
        fast: { schedule: '*/10 * * * *', mode: 'fast', path: '/api/avec/sync' },
        full: { schedule: '5 * * * *', mode: 'full', path: '/api/avec/sync?mode=full' },
        cadence:
          'fast a cada 10 min + full 1×/hora (backup) — tempo real via webhook Avec',
      },
      last,
