...
/** Full parcial/error: retry mais cedo (não esperar 5h). */
const FULL_RETRY_AFTER_FAIL_MS = 45 * 60_000
    ...
    if (last?.created_at) {
      const minGap =
        opts?.webhook && effectiveMode === 'fast'
          ...
          : effectiveMode === 'full'
            ? last.status === 'partial' || last.status === 'error'
              ? FULL_RETRY_AFTER_FAIL_MS
              : FULL_MIN_GAP_MS
            : FAST_MIN_GAP_MS
